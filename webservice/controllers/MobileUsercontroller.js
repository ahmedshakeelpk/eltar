  ...
  form.parse(req, function (err, fields, files) {  
    ...
    userData.save()
      .then(() =>  
        ...
        { 
          if (err) {
            console.log('DriverSignup error--', err);
            var errorMsg	=	"";
            var lang	=	req.body.lang;
              switch (err.name) { 
                  case 'ValidationError':
...
function taxiParcelOfferList(req, res, next){
  ...
  // console.log('./taxiParcelOfferList');
  ...
  searchTaxi.validate()
    .then(() =>{   
      ...
      .populate({
        path: 'driver_id',select: 'full_name phone_number fcm_token'
      }) 
...
router.post('/UpDateToken', function(req, res) {
  ...
  AuthenticateWithToken(req.headers, function(error) {
    ...
    SingleUser.findOne({_id: id}, function(err, user) {
      if (user) {
        console.log(user);
        ...
        user.save(function (error, result) {
          if (!error) {
            ...
            return res.status(200).send({success: true, msg: 'Updated FCM Token.' + user.fcm_token});
          }
          else{
            console.log('Update token error: \n', error);
            return res.status(303).send({success: false, msg: 'Fail to update token'});
          }
        });
...
router.post('/GetAllDrivers', function(req, res) {
 let driver_id = req.body.self_id;

  Chat.find({$or:[{receiverid: driver_id },{ senderid: driver_id}]}, function(err, rr){
    var clients = [];
    if (err) {
      
      return rr.status(401).send({success: false, msg: 'Failed'});
    }
    else {
      for(var i=0; i<rr.length; i++){
        //console.log(rr[i].receiverid);
        //console.log(rr[i].senderid);
        clients.push(rr[i].senderid);
	clients.push(rr[i].receiverid)
      }
    }
    ...
    console.log(clients);
    SingleUser.find({_id: { $in : clients }}, function (e, r) {
      if (e) {
        return res.status(401).send({success: false, msg: 'Failed'});
      } else {
        console.log(r)
        if (r.length === 0) return res.status(200).send({success: true, data: []});
        console.log(r);

        for (var i=0;i<r.length; i++){
          console.log(r[i]._id);
          r[i].fcm_token = r[0].fcm_token || "" ;
          r[i].message_type = r[0].message_type || "";
          r[i].status = r[0].status || "";
          r[i].username = r[0].username || "";
          r[i].carrier_type = r[0].carrier_type || "";
          r[i].lastmessage = r[0].lastmessage ||"";

        }
        return res.status(200).send({success: true, data: r});
      }
    })
  })
});
  ...
  Chat.find({$or:[{receiverid: driver_id},  {senderid: driver_id}]}, function(err, rr){
    ...
    SingleUser.find({_id:{$in: clients}}, function (e, r) {
      if (e) {
        ...
        console.log('here rrrrrrrrrrrrrrrr');
...
router.post('/SendMessage', function(req, res) {
  AuthenticateWithToken(req.headers, function(error) {
    ...
    let file_name = Math.floor(Date.now() / 1000);
    let furl = global.DOCUMENT_UPLOADS_ROOT_PATH;
    ...
    form.parse(req, function(err, fields, files) {
      ...
      // console.log(fields);
      console.log("parseeeeeeeeeeeeeeeeeeeeesssepars");
      let sender_id = fields.sender_id;
      let receiver_id = fields.receiver_id;
      let sender_token = fields.sender_token;
      let receiver_token = fields.receiver_token;
      let message = fields.message;
      let title = fields.title;
      let usertype = fields.usertype;
      let content_type = fields.Content_type;
      let latitude = fields.latitude;
      let longitude = fields.longitude;

      console.log(content_type);
      rid = SingleUser.count({_id: receiver_id}, function(err, c){
          console.log('Count is : ', c);
          return c;

      });
      sid = SingleUser.count({_id: sender_id}, function(err, c){
        console.log('Count is : ', c);
        return c;

      });

      if (!rid || !sid || rid =='undefined' || sid =='undefined'){
        return res.json({success: false, msg: 'Doesn\'t receiver_id or sender_id'});
      }

      // if (SingleUser.count({_id: receiver_id}) === 0 || SingleUser.count({_id: receiver_id}) === 0){
      //   return res.json({success: false, msg: 'Doesn\'t receiver_id or sender_id'});
      // }

      // try{
      let voice = fields.title ==='voice' ? global.BASEURL + furl.replace(global.WEBSITE_ROOT_PATH,'') + file_name +'.' + files.voice.name.split('.')[1]: '';
      let pic = fields.title ==='pic' ? global.BASEURL + furl.replace(global.WEBSITE_ROOT_PATH,'') + file_name + '.' + files.pic.name.split('.')[1]: '';
      // }
      // catch (e) {
	...
	'longitude': longitude
      ...
      newChat.save(function(err, chat) {
        ...
        /// change user table For sender
        SingleUser.findOne({_id: sender_id}, function(e, r) {
        ...
        /// change user table For receiver
        SingleUser.findOne({_id: receiver_id}, function(ee, rr) {
          if (!rr) return res.json({success: false, msg: 'Can not save message'});
          rr.carrier_type = 'receiver';
          rr.message_type = title;
          rr.status = 'sent';
          if(title === 'message')
            rr.lastmessage = message;
          rr.save(function (er, re) {
            if (!re) res.json({success: false, msg: 'Can not save message'});

          })
        });
        sendFCMNotification(receiver_token, fcm_data);
        return res.status(200).send({success: true, msg: 'Success.', data: res_1});
      });
    ...
    form.on('error', function(err) {
      console.log("eroorrrrrrrrrrrrrrrrr");
      console.error(err);
    });
    form.on('progress', function(bytesReceived, bytesExpected) {
      console.log("progresssssssssssssssssssssss");
      var percent_complete = (bytesReceived / bytesExpected) * 100;
      console.log(percent_complete.toFixed(2));
    });
    form.on('end', function(fields, files) {
      console.log("endddddddddddddddddddddddd");
      ...
      // console.log(this);
        ...
        console.log(furl + file_name);
        fs.copy(temp_path, furl + file_name, function(err) {
...
router.post('/GetIndividualChatSeen', function(req, res) {
  AuthenticateWithToken(req.headers, function(error) {
    ...
    console.log('/GetIndividualChatSeen');

    Chat.find( {senderid: receiver_id, receiverid: sender_id,
        seen: {$in:['sent','MessageDelivered', 'MessageSeen']}},
      function (err, chats) {
    ...
    SingleUser.find({ _id: {$in:[receiver_id, sender_id]} },
      function (err, users) {
      if (err){
        console.log(err);
        ...
        for (let i = 0; i < users.length; i ++) {
          let user = users[i];
          user.seen = 'MessageSeen';
          user.save();
        }
    });

    Chat.find( {senderid: {$in: [sender_id, receiver_id]}, receiverid: {$in: [sender_id, receiver_id]},
      seen: {$in:['sent','MessageDelivered', 'MessageSeen']}},
      function (err, chats) {
        if (err) return res.status(400).send({success: false, msg: 'Failed.'});
        return res.status(200).send({success: true, msg: 'Success.', data: chats});
      })
  });
...
router.post('/SendAlert', function(req, res) {
  AuthenticateWithToken(req.headers, function(error) {
    ...
    if ('MessageSeen' === title || 'MessageDelivered' === title){
      Chat.find( {senderid: receiver_id, receiverid: sender_id},
        function (err, chats) {
          if (err) return res.status(400).send({success: false, msg: 'Failed.'});
  ...
  var fcm_data = {
    ...
    sendFCMNotification(receiver_token, fcm_data);
    return res.status(200).send({success: true, msg: 'Success', data: fcm_data});
  });
...
sendFCMNotification = function (receiver_token, msg) {
  let fcm = new FCM(global.FCM_SERVER_KEY);
  var message = {
    to : receiver_token,
    data : {
    ...
    notification : {
      title : msg.title,
      body : msg,
      content_available: msg.Content_type
    }
  };
  fcm.send(message, function(err,response){
  });
